...
function mockResourceSample(): ResourceSample {
  ...
  return {
    timestamp: Date.now(),
    cpu: { loadPercent: 28 + Math.sin(phase) * 12, temperatureC: 58, cores: Array.from({ length: 12 }, (_, index) => 18 + Math.sin(phase + index) * 14) },
    memory: { usedBytes: 12.7 * 1024 ** 3, totalBytes: 31.8 * 1024 ** 3, activeBytes: 10.9 * 1024 ** 3 },
    gpu: { available: true, name: "NVIDIA GeForce RTX 3050 Ti Laptop GPU", loadPercent: 42 + Math.cos(phase) * 18, temperatureC: 61, memoryUsedMiB: 2180, memoryTotalMiB: 4096, powerW: 47 },
